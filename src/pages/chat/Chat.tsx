...
import MessageFileIndicator from '../../components/FileUpload/MessageFileIndicator/MessageFileIndicator'
...
const Chat = () => {
  ...
  const processResultMessage = (resultMessage: ChatMessage, userMessage: ChatMessage, conversationId?: string) => {
    ...
    setMessages([
      ...
      ...(isEmpty(toolMessage) ? [] : [toolMessage]),
      ...(isEmpty(assistantMessage) ? [] : [assistantMessage]),
    ])
  }
  ...
  const updateCurrentChatAfterCosmoDBRequest = ({
    conversationId,
    abortController,
    result,
    userMessage
    ...
    if (conversationId) {
      ...
      resultConversation.messages.push(...newMessages);
    } else {
      resultConversation = {
  ...
  // File Upload
  const makeApiRequestWithCosmosDB = async (question: string, conversationId?: string, file?: File | null) => {
    setIsLoading(true)
    setShowLoadingMessage(true)
    const abortController = new AbortController()
    abortFuncs.current.unshift(abortController)
    ...
    try {
      ...
      if (response?.body) {
        ...
        updateCurrentChatAfterCosmoDBRequest({
          conversationId,
          abortController,
          result,
          userMessage,
        ...
        <Stack horizontal className={styles.chatRoot}>
          <div className={styles.chatContainer}>
            {!messages || messages.length < 1 ? (
              ...
              <div className={styles.chatMessageStream} style={{ marginBottom: '40px' }} role="log">
                {messages.map((answer, index) => (
                  <>
                    {answer.role === 'user' ? (
                      <div className={styles.chatMessageUser} tabIndex={0}>
                        {/* File Upload */}
                        {/* <MessageFileIndicator fileName={answer.fileName} /> */}
            ...
            <Stack horizontal className={styles.chatInput}>
              ...
              <QuestionInput
                ...
                disabled={isLoading}
                onSend={(question, id, file) => {
                  appStateContext?.state.isCosmosDBAvailable?.cosmosDB
                    ? makeApiRequestWithCosmosDB(question, id, file)
